...
async function handleRegistrationSubmit() {
    ...
    try {
        ...
        const race = $("#b-reg-race").val(); // 获取原始选项，"自设" 或 "魅魔"
        let generatedData = {};
        ...
        if (race === "魅魔") {
            // 从已导入的 succubusPrompts 对象中获取
            basePromptTemplate = succubusPrompts.prompt;
        } else if (race === "自设") {
            // 假设您在 prompts.js 中也导出了 customRacePrompts (根据您的文件内容)
            // 如果没有，需要您在 prompts.js 中添加并导出它
            const { customRacePrompts } = await import("./prompts.js");
            basePromptTemplate = customRacePrompts.prompt;
        }
        // 对于人类，可以不传递模板或传递一个空字符串
        ...
        // 3. 使用修正后的函数签名，正确调用 buildSystemWrapper
        const finalPrompt = buildSystemWrapper(
            formData.fixed.race, // 使用 gatherFormData 格式化后的种族名
            basePromptTemplate,
            customExtra,
            obsceneSettings,
        );
