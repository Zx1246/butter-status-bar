...
function renderTopBar(state, context) {
    ...
    // 从 state 中获取时间，如果不存在则提供一个安全的默认值
...
function renderWombStatusText(state) {
    ...
    switch (phase) {
        case "生理期":
            ...
            totalInPhase = lengths.menstrual;
            break;
        case "卵泡期":
            dayInPhase = currentDayInCycle - lengths.menstrual;
            totalInPhase = lengths.follicular;
            break;
        case "排卵期":
            dayInPhase =
                currentDayInCycle - (lengths.menstrual + lengths.follicular);
            totalInPhase = lengths.ovulation;
            break;
        case "黄体期":
            dayInPhase =
                currentDayInCycle -
                (lengths.menstrual + lengths.follicular + lengths.ovulation);
            totalInPhase = lengths.luteal;
            break;
    }
